...
export default function GamePage({
  ...
  useEffect(() => {
    ...
    const fetchAllQuestions = async () => {
      ...
      try {
        const response = await fetch(`/api/emotion/quiz?mode=${gameMode}`)
        if (!response.ok) throw new Error('Spring Boot 서버에서 퀴즈를 가져오지 못했습니다.')
        const data = await response.json()
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setIsQuestionChanging(false), 800)
  ...
  useEffect(() => {
    if (currentQuestion?.text && !isQuestionChanging) speakQuestion(currentQuestion.text)
  }, [currentQuestion, isQuestionChanging])

  useEffect(() => {
    if (feedback !== 'correct') return
    const timeoutId = setTimeout(() => handleNext(), 1500)
    ...
    const captureAndAnalyze = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setDetected(EMOTION_THEMES[emotionKey] || EMOTION_THEMES.neutral)
